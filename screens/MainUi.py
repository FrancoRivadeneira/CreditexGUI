        ...
        while self._FLAG_run and not (exit_flag.is_set()):
            try:
                if not (self.falseParent._FLAG_socketConected):
                    ...
                    _VAR_socketRobot = socket.socket(
                ...
                logging.info(
                    "Se perdió conexión. Volviendo a conectar con el socket [ERROR:{}]".format(e)+"No fuca")
...
class Worker(QtCore.QObject):
    ...
    def imu(self):
        ...
        while True:
            time.sleep(0.05)
            # Supongamos que estos son los nuevos datos
            data = f"{ROLL};{PITCH};{YAW};{SPEED}"
            # print(f"llego estoooooaaaaaaaaa: {data}")
            # Emitimos la señal con los nuevos dato
            self.update_label.emit(data)
...
class MainUi (QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        loadUi(
            "C:/Users/Franco Rivadeneira/Desktop/PlantasIndustrialesV2/screens/interfaceMOD.ui", self)
        ...
        # Conectamos la señal a la ranura
        self.worker.update_label.connect(self.updateYaw)
       ...
       # Se abre maximizando la pantalla
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print("Error: No se pudo conectar al URL. Código de estado:",
        ...
        try:
            ...
            if response.status_code == 200:
                # Si la solicitud fue exitosa, continuar con el proceso de conexión
                self.video_thread2 = VideoStreamThread(
                    url2, self.dimension_updater2)
        ...
        try:
            ...
            if response.status_code == 200:
                # Si la solicitud fue exitosa, continuar con el proceso de conexión
                self.video_thread3 = VideoStreamThread(
                    url3, self.dimension_updater3)
                self.video_threadB = VideoStreamThread(
                    url3, self.dimension_updaterB)
                self.video_threadB_P = VideoProcessThread(
                    self.dimension_updaterB_P)
            else:
                # Si la solicitud no fue exitosa, imprimir un mensaje de error
                print("Error: No se pudo conectar al URL. Código de estado:",
                      response.status_code)
...
########################################## Galeria  #################################################################
        self.btn_fotos.clicked.connect(
            lambda: self.load_images_galery("imagenes"))
        ...
        self.open_close_side_bar_btn.clicked.connect(
        ...
        # Función Click
        self.VisionCamara2.mousePressEvent = lambda event: self.change_camera(
            event, id_sec)
        self.VisionCamara3.mousePressEvent = lambda event: self.change_camera2(
            event, id_sec)
        self.VisionCamara4.mousePressEvent = lambda event: self.change_camera3(
            event, id_sec)
    ...
    def initialize_comboBox(self):
        ...
        pixmap = QPixmap(USERPICKPATH)
        pixmap = pixmap.scaled(
            360, 360, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation)
        icon = QIcon(pixmap)
    ...
    def load_files_galery(self):
        ...
        for i, subdir in enumerate(subdirectories):
            ...
            # Create button
            button = QPushButton()
            # Replace 'carpeta.svg' with your icon file
            button.setIcon(QIcon('iconos/carpeta.svg'))
            # Adjust icon size as needed
            button.setIconSize(QtCore.QSize(300, 300))
            button.clicked.connect(
                lambda checked, img=f"imagenes/{subdir}": self.load_images_galery(img, True))
    ...
    def send_message(self):
        ...
        try:
            ...
            if data:
                ...
                print(
                    f"el numero es:{angle1}+{angle2}+{angle3}+{angle4}+{angle5}+{angle6}")
        ...
        if picture_directory:
            ...
            if id_gen == 1:
                if id_sec == 3:
                    ...
                    self.video_thread3.update_frame.connect(
                        self.display_resized_frame1)
                    self.video_thread3.start()
                    print("llegue")
        ...
        if picture_directory:
            ...
            if id_gen == 1:
                if id_sec == 4:
                    ...
                    self.video_thread4.update_frame.connect(
                        self.display_resized_frame1)
                    self.video_thread4.start()
                    print("llegue")
...
# Funciones para abrir y procesar archivos
        ...
        if picture_directory:
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Si la solicitud no fue exitosa, imprimir un mensaje de error
                    print(
                        "Error: No se pudo conectar al URL. Código de estado:", response.status_code)
            except requests.exceptions.Timeout:
                # Manejar el caso en el que la conexión tarda demasiado en establecerse
                print(
                    f"Error: Tiempo de espera agotado para la conexión a {url}")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.video_threadB.update_frame.connect(
                        self.display_resized_frameB)
                    self.video_threadB.start()
                    self.video_threadB_P = VideoProcessThread(
                        self.dimension_updaterB_P)
                    self.dimension_updaterB_P.dimensions_changed.emit(400, 300)
                    self.video_threadB_P.update_frame.connect(
                        self.display_resized_frameB_P)
            ...
            try:
                ...
                self.video_thread4 = VideoThermalThread(
                    self.dimension_updater4)
                self.dimension_updater4.dimensions_changed.emit(200, 150)
                self.video_thread4.update_frame.connect(
                    self.display_resized_frame4)
                self.video_thread4.start()
    ...
    def generate_report(self):
        ...
        document.paragraphs[-1].add_run().add_picture(image_path,
                                                      width=Inches(2.0))
        ...
        inspection_title = document.add_heading(
            'Registro de Inspección', level=1)
...
# FUNCION PARA REAJUSTAR DISTRIBUCION DE FUNCIONES EN LA PANTALLA PRINCIPAL
    ...
    def GoGenerarReport(self):
        ...
        self.main_body_left.setMaximumSize(10000, 100000)
        # Contiene a frame_general y a frame_mapa
        self.main_frames_login.setMaximumSize(0, 0)
        ...
        self.frame_left_down.setMaximumSize(10000, 10000)
    ...
    def distribuir_secret(self):
        ...
        # Frames Generales
        self.main_body_left.setMaximumSize(
            10000, 10000)  # Contiene frame_left_up
        # Contiene a frame_general y a frame_mapa
        self.main_frames_login.setMaximumSize(0, 0)
        # Contiene a frame_arm, frame_position y frame_secret
        self.frame_left_up.setMaximumSize(2000, 1240)
        # Contiene a frame_arch, frame_documents y derivados del reporte
        self.frame_left_down.setMaximumSize(0, 0)
    ...
    def distribuir_general(self):
        ...
        # Contiene a frame_general y a frame_mapa
        self.main_frames_login.setMaximumSize(10000, 10000)
        # Contiene a frame_arm, frame_position y frame_secret
        self.frame_left_up.setMaximumSize(0, 0)
        # Contiene a frame_arch, frame_documents y derivados del reporte
        self.frame_left_down.setMaximumSize(0, 0)
...
###############################################
    ...
    def distribuir_archivos(self):
        ...
        self.frame_left_up.setMaximumSize(0, 0)
        # Contiene a frame_arch, frame_documents y derivados del reporte
        self.frame_left_down.setMaximumSize(100000, 10000)
    ...
    def reset_button(self):
        ...
        self.main_body_left.setMaximumSize(0, 0)  # Contiene frame_left_up
        # Contiene a frame_general y a frame_mapa
        self.main_frames_login.setMaximumSize(100000, 10000)
        # Contiene a frame_arm, frame_position y frame_secret
        self.frame_left_up.setMaximumSize(0, 0)
        # Contiene a frame_arch, frame_documents y derivados del reporte
        self.frame_left_down.setMaximumSize(0, 0)
    ...
    def saveimages(self):
        ...
        cap.release()
        print(
            f"Se han guardado {frame_count} frames en la carpeta {output_folder}")
