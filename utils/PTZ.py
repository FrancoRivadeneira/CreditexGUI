...
class PTZ(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.url = f"http://{ip}:{PTZ_PORT}/ptz_stream"
        self.control_url = f"http://{ip}:{PTZ_PORT}/ptz_control"
            ...
            if response.status_code == 200:
                ...
                for chunk in response.iter_content(chunk_size=1024):
                    ...
                    if a != -1 and b != -1:
                        ...
                        cv_img = cv2.imdecode(np.frombuffer(jpg, dtype=np.uint8),
                                              cv2.IMREAD_COLOR)
                        if cv_img is not None:
                            self.change_pixmap_signal.emit(cv_img)
            else:
                logging.error(f"Error al conectar con {self.url}, código de estado: {response.status_code}")  # noqa
    ...
    def send_control_command(self, command):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.error(
                    f"Error al enviar comando {command}, código de estado: {response.status_code}")
        except Exception as e:
