    ...
    def run(self):
        ...
        if True:
            try:
                ...
                stdin, stdout, stderr = self.sshForStop.exec_command(
                ...
                logging.info(
                    "Conexion por ssh establecida para la grabacion. ")
                ...
                self.connection_status_signal.emit(
                    "<font color='green'>>Se detuvo la grabacion de los archivos de video y bag, se recomienda usar el boton info para ver los estados de sus archivos...</font>")
                self.falseParent._sshStopConnected = True
                _VAR_counter = 1
            except Exception as e:
                logging.error(f"Error en la conexión SSH: {e}")
                self.connection_status_signal.emit(
                    "<font color='red'>>Hubo un problema con la conexion a ethernet intente volver a conectar...</font>")
            finally:
